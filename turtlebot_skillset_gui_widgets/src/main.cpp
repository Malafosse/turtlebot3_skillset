...
bool parse_arg(int argc, const char* argv[])
{
    ...
    try
    {
        ...
        manager_node_name = args["manager"].as<std::string>("turtlebot_skillset");
        return true;
    }
...
struct MainGui: gui_tools::App
{
    ...
    : gui_tools::App(title, w, h, false, false)
    {
        ...
        widget = std::make_shared<TurtlebotSkillsetWidget>(manager_node_name, node,
    ...
    bool update();
    std::shared_ptr<TurtlebotSkillsetWidget> widget;
};
...
int main(int argc, char ** argv)
{
    ...
    // ROS
    auto node = std::make_shared<rclcpp::Node>("turtlebot_skillset_gui");
    rclcpp::executors::SingleThreadedExecutor executor;
    executor.add_node(node);

    // Gui
    MainGui gui(node, "Turtlebot Skillset GUI", 1024, 756);
